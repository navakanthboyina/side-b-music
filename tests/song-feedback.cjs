...
w.eval(fs.readFileSync(root+'data.js','utf8'));w.eval(fs.readFileSync(root+'app.js','utf8').replaceAll("import('./ai-client.mjs?v=lightweight-1')","Promise.resolve(window.__client)").replaceAll("import('./ai-core.mjs')","Promise.resolve(window.__core)"));
return {dom,w,d};}
...
scenario='failure';d.querySelector('#refresh').click();await new Promise(r=>setImmediate(r));assert.match(d.querySelector('#feed').textContent,/New Song/);assert(!d.querySelector('#refresh').disabled);
