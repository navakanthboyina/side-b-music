...
w.eval(fs.readFileSync(root+'data.js','utf8'));w.eval(fs.readFileSync(root+'app.js','utf8').replaceAll("import('./ai-client.mjs?v=mixed-12-1')","Promise.resolve(window.__client)").replaceAll("import('./ai-core.mjs?v=mixed-12-1')","Promise.resolve(window.__core)"));
return {dom,w,d};}
let {dom,w,d}=boot(JSON.stringify(base));
assert.equal(d.querySelectorAll('[data-week]').length,2);
assert.equal(d.querySelectorAll('#plan-cards .music-card').length,6);
assert.equal(d.querySelector('#language'),null);
for(const week of [1,2])assert(new Set(w.SIDE_B_DATA.artists.filter(a=>a.week===week).map(a=>a.language)).size>=3);
d.querySelector('[data-week="2"]').click();assert.equal(d.querySelectorAll('#plan-cards .music-card').length,6);
