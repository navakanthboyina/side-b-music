...
function validate(raw){
 ...
 if(raw.filters&&['All languages','Telugu','Tamil','Hindi','English'].includes(raw.filters.language)&&['Any mood','Warm','Reflective','Energetic'].includes(raw.filters.mood))s.filters={language:'All languages',mood:raw.filters.mood};
...
function matches(a){return (state.filters.mood==='Any mood'||a.mood===state.filters.mood);}
function chosen(){const day=new Date().toISOString().slice(0,10);return seeds().filter(matches).sort((a,b)=>((state.artistVisits['artist:'+norm(a.name)]||0)-(state.artistVisits['artist:'+norm(b.name)]||0))||(score(b)+hash(day+state.rotation+b.name)%100/40)-(score(a)+hash(day+state.rotation+a.name)%100/40)).slice(0,4);}
const starters=()=>D.artists.filter(matches).sort((a,b)=>score(b)-score(a)||a.week-b.week).slice(0,12);
function card(a,i,plan=false){return `<article class="music-card"><div class="card-top"><span>${plan?['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY'][i]:esc(a.mood)}</span><span class="card-index">${String(i+1).padStart(2,'0')}</span></div><div class="card-body"><h3>${esc(a.name)}</h3><div class="track-title">${esc(a.track||'Explore their songs')}</div><p>${esc(a.note||'An artist you added to your taste profile.')}</p>${links(a.query||a.name+' official music')}</div>${plan?`<label class="explored"><input type="checkbox" data-explored="${a.id}" ${state.explored.includes(a.id)?'checked':''}> I explored this artist</label>`:''}${buttons(a.name,a.track)}</article>`;}
...
async function unseenCandidates(signal){
 ...
 const selected=[...poolSeeds.values()].filter(matches).sort((a,b)=>(state.artistVisits['artist:'+norm(a.name)]||0)-(state.artistVisits['artist:'+norm(b.name)]||0)||hash(state.rotation+a.name)-hash(state.rotation+b.name)).slice(0,6);
 ...
 const queries=await Promise.allSettled(selected.map(async seed=>{
  ...
  }).sort((a,b)=>hash(state.rotation+a.trackName)-hash(state.rotation+b.trackName)).slice(0,4).map(t=>({artist:t.artistName,title:t.trackName,language:seed.language,mood:seed.mood,genre:t.primaryGenreName||'',catalog:t}));
 }));
...
async function refresh(){
 if(busy)return;
 pruneHistory();busy=true;$('#ai-diagnostics').hidden=true;$('#ai-diagnostic-text').textContent='';
 const key=filterKey(), snapshot=profileKey(), ai=state.engine==='ai', btn=$('#refresh');
 btn.disabled=true;$('#clear-ai').disabled=true;$('#recommendation-mode').disabled=true;btn.textContent=ai?'Finding AI picks…':'Finding music…';
 ...
 try {
  if(ai){
   ...
   const [client,core]=await Promise.all([import('./ai-client.mjs?v=mixed-12-1'),import('./ai-core.mjs?v=mixed-12-1')]);
  ...
  save();renderFeed();
  if(ai)$('#ai-status').textContent=fresh.length?`${fresh.length} AI song picks ready. Refresh picks generates another mixed-language batch; these results are saved for your next visit.`:message;
 }catch(err){
  const text=err.message||'Could not generate picks. Please try again.';
  if(ai)$('#ai-status').textContent=text;
  if(err.diagnostics){$('#ai-diagnostic-text').textContent=JSON.stringify(err.diagnostics,null,2);$('#ai-diagnostics').hidden=false;}
  $('#feed-status').textContent=text+' Existing results have not been replaced.';
 }finally{
  aiController=null;busy=false;btn.disabled=false;$('#clear-ai').disabled=false;$('#recommendation-mode').disabled=false;$('#cancel-ai').hidden=true;
...
$('#recommendation-mode').addEventListener('change',()=>{
 ...
 if(state.engine==='catalog')import('./ai-client.mjs?v=mixed-12-1').then(m=>m.stop()).catch(()=>{});
});
$('#clear-ai').addEventListener('click',async()=>{
 ...
 try{const client=await import('./ai-client.mjs?v=mixed-12-1');await client.clearDownloads();$('#ai-status').textContent='AI downloads cleared. Your song ratings are unchanged. Click Refresh picks to load Lightweight AI.';}
...
$('#today').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});$('#mood').value=state.filters.mood;
for(const id of ['mood'])$('#'+id).addEventListener('change',()=>{state.filters[id]=$('#'+id).value;mode='starter';message='Filters updated. Refresh picks for live catalog results.';save();renderFeed();});
