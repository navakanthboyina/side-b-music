...
async function refresh(){
 ...
 try {
  if(ai){
   ...
   const [client,core]=await Promise.all([import('./ai-client.mjs?v=candidate-formats-1'),import('./ai-core.mjs?v=candidate-formats-1')]);
...
$('#recommendation-mode').addEventListener('change',()=>{
 ...
 if(state.engine==='catalog')import('./ai-client.mjs?v=candidate-formats-1').then(m=>m.stop()).catch(()=>{});
});
$('#clear-ai').addEventListener('click',async()=>{
 ...
 try{const client=await import('./ai-client.mjs?v=candidate-formats-1');await client.clearDownloads();$('#ai-status').textContent='AI downloads cleared. Your song ratings are unchanged. Click Refresh picks to load Lightweight AI.';}
