...
const blank=()=>({version:1,engine:'ai',shown:{},artistVisits:{},rotation:0,ratings:{},explored:[],imports:[],seeds:[],filters:{language:'All languages',mood:'Any mood'},cache:null});
...
const getRating=name=>state.ratings[norm(name)]?.value;
function setRating(name,value){if(typeof name!=='string'||!norm(name)||name.length>300||!['replay','skip','known','clear'].includes(value))throw Error('Invalid artist or rating.');const k=norm(name);if(value==='clear')delete state.ratings[k];else state.ratings[k]={name,value};state.cache=null;save();render();toast('Taste updated. Refresh picks to explore the change.');return {artist:name,rating:getRating(name)||null};}
...
function buttons(name){return `<div class="card-feedback">${[['replay','♡ Replay'],['skip','Skip'],['known','Know them']].map(([v,t])=>`<button class="rate" data-artist="${esc(name)}" data-rating="${v}" aria-label="${esc(t+' '+name)}" aria-pressed="${getRating(name)===v}">${t}</button>`).join('')}</div>`;}
...
function score(a){let s=1+(a.origin&&a.origin!=='starter'?3:0)+(getRating(a.name)==='replay'?5:0)-(getRating(a.name)==='known'?1:0);for(const r of Object.values(state.ratings)){const ref=D.artists.find(x=>norm(x.name)===norm(r.name));if(ref?.mood===a.mood)s+=r.value==='replay'?1:r.value==='skip'?-.5:0;}return s;}
function hash(s){let h=2166136261;for(const c of s){h^=c.charCodeAt(0);h=Math.imul(h,16777619);}return h>>>0;}
function matches(a){return getRating(a.name)!=='skip'&&(state.filters.language==='All languages'||a.language===state.filters.language)&&(state.filters.mood==='Any mood'||a.mood===state.filters.mood);}
...
function renderProfile(){$('#import-count').textContent=state.imports.length;$('#liked-count').textContent=Object.values(state.ratings).filter(r=>r.value==='replay').length;$('#explored-count').textContent=state.explored.length;const counts=new Map();state.imports.forEach(t=>t.artist.split(/\s*;\s*/).forEach(n=>counts.set(n,(counts.get(n)||0)+1)));const top=[...counts].sort((a,b)=>b[1]-a[1]).slice(0,8);$('#taste-summary').innerHTML=top.length?`<p class="small muted">Most represented artists in your imports</p><div class="chips">${top.map(([n,c])=>`<span class="chip">${esc(n)} · ${c}</span>`).join('')}</div>`:'<p class="muted">Confirmed: English, Telugu, Hindi, and Tamil. Genres and favorite artists are unverified until you import or add them.</p>';$('#seed-list').innerHTML=state.seeds.map((a,i)=>`<span class="chip">${esc(a.name)} · ${esc(a.language)}<button data-remove-seed="${i}" aria-label="Remove ${esc(a.name)}">×</button></span>`).join('');const rates=Object.values(state.ratings);$('#ratings-list').innerHTML=rates.length?rates.map(r=>`<div class="rating-row"><div><strong>${esc(r.name)}</strong><br><span>${({replay:'More like this',skip:'Skipped from fresh picks',known:'Already familiar'})[r.value]}</span></div><button class="secondary" data-artist="${esc(r.name)}" data-rating="clear">Undo</button></div>`).join(''):'<p class="muted">Rate an artist in Discover or your four-week plan to start.</p>';$('.side-sources .small').textContent=state.imports.length?`${state.imports.length} songs imported on this device. No Spotify sync.`:'Links connected. Songs haven’t been imported.';}
...
const profileKey=()=>JSON.stringify([state.filters,state.ratings,state.imports,state.seeds,state.engine]);
async function refresh(){
 ...
 try {
  if(ai){
   ...
   selected=selected.filter(a=>matches(a)&&getRating(a.name)!=='known');
   $('#ai-status').textContent='AI artists selected. Checking the catalog for real tracks…';
  ...
  const results=await Promise.allSettled(selected.map(async seed=>{
   ...
   const seen=new Set(),found=d.results.filter(t=>{
    ...
    if(!hit||seen.has(k)||recentlyShown({artist:t.artistName,title:t.trackName}))return false;
    seen.add(k);return true;
   });
   const day=new Date().toISOString().slice(0,10);
   found.sort((a,b)=>hash(day+state.rotation+a.trackName)-hash(day+state.rotation+b.trackName));
   return found.slice(0,3).map(t=>({artist:t.artistName,title:t.trackName,seed:seed.name,language:seed.language,mood:seed.mood,ai,
    year:/^\d{4}-/.test(t.releaseDate||'')?t.releaseDate.slice(0,4):'',
    reason:ai?'AI fit: '+seed.reason:getRating(seed.name)==='replay'?'Because you chose Replay for '+seed.name:seed.origin==='import'?'From an artist in your imported playlists':seed.origin==='added'?'From an artist you added':'Exploring your provisional '+seed.language+' '+seed.mood.toLowerCase()+' direction'}));
  }));
  ...
  const batchKeys=new Set();fresh=fresh.filter(t=>{const k=trackKey(t);if(batchKeys.has(k)||recentlyShown(t)||!matches({name:t.seed,language:t.language,mood:t.mood}))return false;batchKeys.add(k);return true;});
  if(fresh.length){
   items=fresh;mode='live';const at=Date.now();for(const t of items)state.shown[trackKey(t)]=at;state.cache={at,filter:key,items};
   message=`Checked ${new Date(at).toLocaleString(undefined,{dateStyle:'medium',timeStyle:'short'})} · ${ai?'Local AI artist discovery + ':''}iTunes metadata · ${failed?'Some searches unavailable.':'Listen through the links below.'}`;
  }else{
...
document.addEventListener('click',e=>{const r=e.target.closest('[data-rating]');if(r){setRating(r.dataset.artist,getRating(r.dataset.artist)===r.dataset.rating?'clear':r.dataset.rating);return;}const w=e.target.closest('[data-week]');if(w){week=Number(w.dataset.week);renderPlan();$('.week-tabs [data-week="'+week+'"]').focus();}const s=e.target.closest('[data-remove-seed]');if(s){state.seeds.splice(Number(s.dataset.removeSeed),1);state.cache=null;save();render();}});
...
if(document.modelContext?.registerTool){const lifecycle=new AbortController();try{Promise.resolve(document.modelContext.registerTool({name:'rate_music_artist',description:'Set or clear a local Munna’s Grooves artist preference and update the dashboard.',inputSchema:{type:'object',properties:{artist:{type:'string',minLength:1,maxLength:300},rating:{type:'string',enum:['replay','skip','known','clear']}},required:['artist','rating'],additionalProperties:false},annotations:{readOnlyHint:false},execute(input){if(!input||typeof input.artist!=='string'||!['replay','skip','known','clear'].includes(input.rating))throw Error('Artist and valid rating required');return setRating(input.artist,input.rating);}},{signal:lifecycle.signal})).catch(()=>{});}catch{}window.addEventListener('pagehide',()=>lifecycle.abort(),{once:true});}
})();
